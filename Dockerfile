...
ENV PATH $PREFIX_DIR/bin:$PREFIX_DIR/sbin:$PATH
...
ENV PHP_INI_DIR $PREFIX_DIR/etc/php
...
COPY docker-php-source $PREFIX_DIR/bin/
...
# /usr/src/php/ext/pcre/pcrelib/pcre_jit_compile.c:65:2: error: #error Unsupported architecture
	...
	&& make -j "$(nproc)" \
	&& make install \
	&& { find $PREFIX_DIR/bin $PREFIX_DIR/sbin -type f -perm +0111 -exec strip --strip-all '{}' + || true; } \
	&& make clean \
	&& cd / \
...
COPY docker-php-ext-* docker-php-entrypoint $PREFIX_DIR/bin/
...
RUN apk add --no-cache \
    ...
    docker-php-ext-configure gd \
      ...
      --with-png-dir=$PREFIX_DIR/include/ \
      --with-jpeg-dir=$PREFIX_DIR/include/ \
    && \
    docker-php-ext-install gd \
    && \
    CFLAGS="-I/usr/src/php" docker-php-ext-install -j $(grep -c ^processor /proc/cpuinfo 2>/dev/null || 1) \
    xmlreader \
    && \
...
RUN echo "xdebug.remote_handler=dbgp" >> $PREFIX_DIR/etc/php/conf.d/php.ini
RUN echo "xdebug.remote_host=bash" >> $PREFIX_DIR/etc/php/conf.d/php.ini
RUN echo "xdebug.remote_port=9009" >> $PREFIX_DIR/etc/php/conf.d/php.ini

RUN php -r "copy('https://getcomposer.org/installer', 'composer-setup.php');" \
  && php -r "if (hash_file('SHA384', 'composer-setup.php') === '544e09ee996cdf60ece3804abc52599c22b1f40f4323403c44d44fdfdd586475ca9813a858088ffbc1f233e9b180f061') { echo 'Installer verified'; } else { echo 'Installer corrupt'; unlink('composer-setup.php'); } echo PHP_EOL;" \
  && php composer-setup.php --install-dir=${PREFIX_DIR}/bin --filename=composer \
